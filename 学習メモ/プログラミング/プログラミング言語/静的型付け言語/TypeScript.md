...
- Type Scriptの概要
        ...
        - プログラミング言語
            - 静的型付け言語
                - JavaScriptに型という概念が追加された言語
                    - JSの上位互換的な立ち位置(スーパーセットと呼ばれる)
    - TypeScriptの歴史
        - JavaScriptでも大規模開発をする事ができるように開発されたプログラミング言語
            - 大規模開発において大人数の開発においても高い水準、可読性、統一性などを担保するために型という概念が重要であった
                - JSが進化する事で解決するのではないか？と考えられるが
                    - そうならなかった
                        - コンパイル言語という特性上解決できなかった
            - 型
                - TyprScriptは、JavaScriptにトランスパイルされてJavaScriptになり機械語へ翻訳される
                    - この時に型という概念が重要になる
                - 型推論
                - 型注釈
                - 型システム
                    - 数学的に型を定義する事でプログラムの欠陥をカバーする事ができる
        - 1990年 => JS誕生以前
    ...
    - 静的型付け
      - 型の決定するタイミング
        - 処理の実行時に型が決まる言語 => 動的型付け言語 => インタプリンタ言語 => プログラミング言語から機械語への変換
        - コンパイル時に型が決まる言語 => 静的型付け言語 => コンパイル言語 => TypeScriptからJavaScriptへ変換される時に型システムによって判別される
      - 型注釈はコスト、投資と考えるか
        - 変数毎に型注釈を記載する必要がある
          - コストと考える事ができる
    ...
    - TSのエコシステム
      ...
      - 開発ツール
        ...
        - モジュールバンドラー
          - 複数のJSファイルを結合するツール
            - 素のままでは適切な順番で読み込む順を指定する必要があるが、ツールを使う事で解決する事ができる
        - タスクランナー
          - ビルドを自動化してくれるツール
        - トランスパイラー
          - コンパイラーの一種
    ...
    - 開発環境
      - Node.js
        - Node.jsは本来サーバーサイドで動作する事を目的に開発されたソフト
        ...
        - 静的型付け言語
            - コンパイル時に型が決まる言語
                - 静的型付け言語(TypeScript)が実行されるまでの流れ
                    - エディターでコードを記述
                    　↓ *　エディターの補完機能によるチェック
                    - コンパイラー(TSCコンパイラー)によってトランスパイルが実行される
                    　↓ *　ASTによる型のチェックが入る
                    - トランスパイラーによってJavaScriptにトランスパイルされる
            - トランスパイル => 高水準言語から高水準言語への変換
    ...
    - TypeScriptにおけるメモリ管理
      - JavaScriptと同様のメモリ管理を行う
        - JavaScriptのガベレージコレクションを用いて自動的にメモリを解放する
    - TypeScriptを使う理由
...
- TypeScriptの仕様
                ...
                - 関数の処理が完了する場合
                  - void型
                    - 関数の戻り値の型注釈として使用する
                      - 変数の型として使う事は出来ない
                    - undefinedの上位型
                      - voidへundefinedを代入する事はできるが、undefinedへvoidを代入する事は出来ない
                    - 使い方
                      - undefined型とvoid型は明確に使い分けを行う(狭義ではあるが、使い分けを行う)
                        - 関数の中で明確な戻り値を定義しない場合(return文をかかない場合)
                          - void型
                        - 関数の中でundefinedを返す可能性がある関数の戻り値
                          - undefinedと想定される型のユニオン型を使う
                - 関数の処理が完了しない場合や存在しえないデータ型の変数
                  - never型
                    - never型の関数は終了する事がない
                      - 確実にエラーが返却される関数
                      - 終了する事がない関数
                      - 存在しえないデータ型を表現する変数
                        - 上記の場合にnever型を使う
                    - never型にはnever型しか代入する事ができず、その他の型には逆になんでも代入する事が出来る
                    - 使い方
                      - 網羅性チェック(exhaustiveness check)
                        - ユニオン型を分岐処理するとき、ロジックがすべてのパターンを網羅しているかをコンパイラにチェックさせる処理
                          - default分岐を活用して網羅性チェックを行う
                            - もしも、分岐処理において網羅性が担保された処理だった場合には、default分岐に処理が入らない
                              - チェックしたい値をnever型に代入する処理を記述する方法
                              - 網羅性チェックのための例外クラスを作成して、作成したクラスのコンストラクタ引数にnever型を指定する方法
                                - 網羅性が正しくない場合にはTypeScriptはエラーが吐く
                            - 例外を吐く方法が推奨される理由
                              - noUnusedLocalsに対応可能
                                - コンパイラオプションとしてnoUnusedLocalsが存在する
                                  - もしも、変数ねnever型を代入するだけの関数を作成した場合に、上記のコンパイラチェックに引っかかってしまう
                                    - クラスを作成して、コンストラクタの形にする事で生じない
                              - 実行時を意識したコードになる
                                - TypeScriptはトランスパイルされて結局はJavaScriptになります。
                                  - そのため、他の開発者がJavaScriptで上記の処理を見た場合に、意図が不明で単に変数にvalueを代入しているのみの処理となる
                                    - クラスをnewする形で記述する事でJavaScriptになっても理解する事が出来る処理となる
            - **型のエイリアス**
    ...
    - 型推論 => type inference
      - コンパイラが型を推測する
        - 型注釈を必ず書く必要がないため、プログラマによるコード記述量が削減される
          - では、動的な型付け言語と変わりがないのでは？
            - 動的型付け言語も型が自動的に判別されるが、
              - あくまでも、静的型付け言語 => トランスパイル時に型が判別される　という概念に変更はない
                - 動的型付け言語は実行時に型が決まる。
        - 型を明示しなくてもコンパイラによる型チェックが実行されるため、安全性がます
          - TSのコンパイル時におけるエラーは歓迎されるもの
            - エラーと聞くとよくないものという印象が強くあるが、TSはプログラマの代わりにエラーを発見してくれている
          - 型推論は万能ではない
            - コンテキストから型を推論するが、型を判別する事が出来ない事も存在するという事を念頭におく必要がある
    - コンパイルを通す
