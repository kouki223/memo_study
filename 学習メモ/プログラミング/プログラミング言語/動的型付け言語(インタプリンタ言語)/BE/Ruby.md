...
- Rubyの概要
    ...
    - Rubyが使われているシステム例
        - Homebrew
            - パッケージ管理システム
                - mac OS上でパッケージのインストール、アンインストールが一元的に管理する事ができるようになる
- Rubyの仕様
    - 分の区切り
        - 改行
    ...
    - 正規表現　＝＞　regular expression => RubyのRegexpクラスの由来
        ...
        - 正規表現一覧
            ...
            * 直前の文字やパターンが0回以上連続する
            ...
            ^ 行頭を表す
            $ 行末を表す
            \ メタ文字をエスケープしたり、\nや\wといったほかのメタ文字の一部になったりする
        - Rubyで正規表現検索をかける方法
            - matchメソッドに引数として対象を渡す方法
        - キャプチャに名前をつける
            - キャプチャに名前をつけない場合には連番でデータを取得する事になる
                - (?<>)というメタ文字を使う事で名前をつける事ができる
                    - (?<year>\d+)年(?<month>\d+)月(?<day>\d+)日
                        - m['year'] #=> "1977" =>　文字列で指定する
                        - m[2] #=> "7" => 連番で指定
        - =~を使ってキャプチャの名前がそのままローカル変数に割り当てられる
            - 左辺に正規表現リテラルを置き、右辺に文字列を置き=~を使う事でローカル変数にそのまま割り当てられる
                - 左辺と右辺を入れ替えてしまうと使えないので注意が必要
        - scanメソッド
            - 引数で渡した正規表現にマッチする部分を配列に入れて返す
                - 正規表現に()があるとキャプチャされた部分が配列の配列になって帰ってくる
                    - グループ化はしたくないが、キャプチャはしたくない場合
                        - (?:)というメタ文字を使用する
        - [],slice,slice!
            - []
                - []に正規表現を渡すと正規表現にマッチした部分を抜き出す
                    - マッチする部分が複数ある場合には最初にマッチした部分を抜き出す
                        - 第二引数で何番目のキャプチャを取得するか指定する事ができる
            - slice
                - []のエイリアスメソッド
            - slice!
                - マッチした部分が破壊的に取り除かれる
            - split
                - splitに正規表現を渡す事でマッチした文字列を区切り文字にして分解、配列にして返す
            - gsub、gsub!
                - 第1引数の正規表現にマッチした文字列を第2引数の文字列で置き換える
                    - 第2引数にハッシュで変換のルールを指定する事もできる
                        - hash = { ',' => ':', '-' => '/' }
                        - text.gsub(/,¦-/, hash) #=> "123:456/789"
                    - ブロックの戻り値で置き換える文字を指定する事もできる
                        - text.gsub(/,¦-/) { ¦matched¦ matched == ',' ? ':' : '/' } #=> "123:456/789"
                - gsub!は破壊的なメソッド
                    - gsubやgsub!はキャプチャと組み合わせて文字列を置換する事もできる
                        - キャプチャを使う場合
                            - 第2引数に指定する方法
                                - 、\1や\2のようにしてキャプチャした文字列を連番で参照できます
                                    - このとき、有効な連番は\1から\9までです
                                        - シングルクウォーテーションとダブルクウォーテーションで囲む時で注意する事が必要
                                            - ダブルクウォーテーションで囲むとスラッシュ記法と認識されてしまう
                                                - その場合、\\を2つつける
                            - ブロックと組み込み変数を使う方法
                                - ブロックを使って文字列を置換する
                                    - ```Ruby
        ...
        - 正規表現オブジェクトについて詳しくみる
            - 正規表現オブジェクトをつくる方法
                - //
                    - /\d{3}-\d{4}/
                - Regexp.new（エイリアスメソッドはRegexp.compile）の引数にパターンを渡す方法
                    - Regexp.new('\d{3}-\d{4}')
                - %rを使う方法
                    ```Ruby
                    ...
                    %r{https://example\.com}
                    ```
                - / /や%rの中で#{}を使うと式の値を埋め込むことができます。
                    ```Ruby
                    pattern = '\d{3}-\d{4}'
                    # 変数が展開されるので/\d{3}-\d{4}/と書いたことと同じになる
                    '123-4567' =̃ /#{pattern}/ #=> 0
                    ```
                - case節で指定した文字列が条件にマッチすると処理が実行されるようにできる
                    ```Ruby
                    text = '03-1234-5678'
                    case text
                    when /^\d{3}-\d{4}$/
            ...
            - 正規表現にオプションを渡す
                ...
                - mオプション
                    - 任意の文字を表すドット（.）が改行文字にもマッチするようになる
                        - Regexp.newを使う場合は、Regexp::MULTILINEという定数を渡す
                            ...
                            - "Hello\nBye" =̃ regexp #=> 0
                - Xオプション
                    - 空白文字（半角スペースや改行文字）が無視する
                        - #を使って正規表現中にコメントが記載できるようになる
                    ```Ruby
                ...
                - オプションは複数を一度に使う事ができる
                    - "HELLO\nBYE" =̃ /Hello.Bye/im #=> 0 => iとmオプションを同時に使う
                    - Regexp.newを使う場合は　|　で連結する
                        - regexp = Regexp.new('Hello.Bye', Regexp::IGNORECASE | Regexp::MULTILINE)
        - 方言に注意する
            - 正規表現は基本的な考え方は言語間で共通している
                - しかし、メタ文字の定義が違う場合など方言と言われる違いが存在している
            - Webサービスを利用する場合には方言にあったサービスを使う事が勧められる
                - Ruby => Rubular
                - JavaScript => （https://regexr.com/)など
    - 組み込み変数
        - $で始まる特殊な変数が存在する
            - =~演算子やmatchメソッドを使う事で組み込み変数にマッチした結果が代入される
            - ```Ruby
                text = '私の誕生日は1977年7月17日です。'
                # =̃やmatchメソッドを使うとマッチした結果が組み込み変数に代入される
                text =̃ /(\d+)年(\d+)月(\d+)日/
                # MatchDataオブジェクトを取得する
                $̃ #=> #<MatchData "1977年7月17日" 1:"1977" 2:"7" 3:"17">
                # マッチした部分全体を取得する
                $& #=> "1977年7月17日"
                # 1番目～3番目のキャプチャを取得する
                $1 #=> "1977"
                ...
                $+ #=> "17"
                ```
            - $はMatchDataオブジェクトと同等の値をほとんど取得する事ができる
        - 組み込み変数を書き換えないmatch?メソッド
            - 文字列が正規表現にマッチすればtrue、マッチしなければfalseを返す
                - =~演算子やmatchメソッドよりも高速に動作する
    - キャッシュ化
        - Solid Cache
            - cacheを使う事でHTMLをキャッシュに保存する事ができる
                - キャッシュにしたいHTMLタグをラップする
                    - ```Ruby
                        <% cache @product do %>
                            <h1><%= @product.name %></h1>
                        <% end %>
                    ```
                - ＠productにchaheメソッドを渡すと固有のキーを生成する
        - キャッシュを有効にするコマンド
            - $ bin/rails dev:cache
...
[一行パターンマッチを使ってnilを消すテクニック](https://zenn.dev/sanfrecce_osaka/articles/excluding-nil-by-pattern-matching)
# 命題：パターンマッチという記述方法を使ってよりコードで表明して堅牢なコードを書く

- 様々な局面において、nilが入ってしまう事が予想される局面が存在する(想定していない挙動)
    - 認証 => 何らかの失敗によって認証が出来なかった時 => nil(本当は、その時点で例外を吐く事が望ましいが、、、)
        - それらをコード上で明確にブロックする
            - 一行パターンマッチを使う事でより明示的に、少ないコードの量(密度が高く)で、可読性も高く、記述する事ができる

## 表明

[達人プログラマー]
- もし、起きえないというのであればそれを表明を用いて保証する事
    - 「起きえない」と思っている事があれば、それを表明(assertion)を用いて表現する

[リファクタリング]
- 表明はコミュニケーションとしてとデバックとして働いてくれる
    - コミュニケーション
        - プログラムを組み際の書き手がどのような前提になっているのかを示すかつ、堅牢なコードを記述する事ができる
    - デバック
        - バグをより限定的に見つける事ができる
...
- TS
    - assertion function
- Ruby
    - 表明を表す機能はActive Recordに存在しない
        - そのため、パターンマッチを活用する

### Rubyにおけるパターンマッチについて

- パターンマッチを記述する事が推奨される場合
    - 素のRubyを使っている場合、かつ、明らかに例外が存在しえない場合(想定外のエラーのみが考えられる場合)
        - Rubyの一行パターンマッチは左辺が右辺にマッチしなかった場合に、NoMatchingPatternErrorをraiseする
            - 一行存在するだけで、その値がnilではない事を明示的に示す事ができる
- パターンマッチが推奨されない場合
    - railsなどの引数チェックにおいては、表明は使われない想定になっている
        - ユーザーが想定していない型の値を渡す可能性がある場合などがあるため
            - あくまでも表明は、その型が起きえないと示す場合に有効になる
...
**解説**: `[*, AdminUser => admin_user, *]` というパターンマッチは、配列の中から `AdminUser` クラスのインスタンスを探して `admin_user` 変数に束縛します。`*` は任意の要素を表し、前後に何があっても良いことを意味します。マッチする要素がなければ `NoMatchingPatternError` が発生するため、`nil` を返すことはありません。
...
**解説**: `Proc` を使うことで、より複雑な条件でのマッチングが可能になります。`-> (user) { user.role == 'admin' }` は `Proc` オブジェクトで、`Proc#===` が呼ばれて評価されます。配列内の要素に対してこの `Proc` を実行し、`true` を返す最初の要素が `admin_user` に束縛されます。
...
**解説**: ピン演算子 `^` を使うことで、変数や式の評価結果をパターンとして使えます。`:admin?.to_proc` は `admin?` メソッドを呼び出す `Proc` に変換され、配列内の要素に対して `admin?` メソッドが呼ばれます。`admin?` が `true` を返す最初の要素が `admin_user` に束縛されます。

**Array#first・Array#last の場合**
...
**解説**: 
- `[first_user, *]` は配列の最初の要素を `first_user` に束縛します。配列が空だと `NoMatchingPatternError` が発生するため、`nil` を返すことはありません。
- `[*, last_user]` は配列の最後の要素を `last_user` に束縛します。同様に配列が空だとエラーになります。
...
**解説**: `[_, second_user, *]` は配列の2番目の要素を `second_user` に束縛します。`_` は最初の要素を無視することを意味します。配列に2つ以上の要素がない場合は `NoMatchingPatternError` が発生します。
...
**解説**: 複数のパターンを組み合わせた例です。
- `first_user`: 配列の1番目の要素
- `second_user`: 配列の2番目の要素
- `*other_users`: 3番目から最後の1つ前までの要素を配列として束縛
- `AdminUser => last_user`: 配列の最後の要素が `AdminUser` クラスのインスタンスであることを確認し、`last_user` に束縛

すべての条件を満たさない場合は `NoMatchingPatternError` が発生します。
...
**解説**: `{ name: }` は、ハッシュに `:name` キーが存在することを確認し、その値を `name` 変数に束縛します。キーが存在しない場合は `NoMatchingPatternError` が発生するため、`nil` を返すことはありません。`Hash#fetch` と同様の動作ですが、パターンマッチの構文で記述できます。
...
**解説**: 複雑なハッシュのパターンマッチの例です。
- `name:`: `:name` キーの値を `name` 変数に束縛
- `role: :admin | :member => role`: `:role` キーの値が `:admin` または `:member` のいずれかであることを確認し、`role` 変数に束縛
- `age: 0..30`: `:age` キーの値が 0〜30 の範囲内であることを確認（変数には束縛しない）
- `books: [*, { title: /[Rr]uby( on Rails)?/ => ruby_book_title }, *]`: `:books` キーの配列の中から、`title` キーの値が正規表現 `/[Rr]uby( on Rails)?/` にマッチするハッシュを探し、そのタイトルを `ruby_book_title` 変数に束縛
- `**others`: 上記以外のすべてのキーと値のペアをハッシュとして `others` 変数に束縛
